...
define('SUPABASE_URL', 'https://ayrbykshrecdewwdlxdc.supabase.co');
define('SUPABASE_KEY', 'your_supabase_api_key_here'); // Replace with your actual API key
define('DB_HOST', 'db.ayrbykshrecdewwdlxdc.supabase.co');
...
try {
    $supabase_conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME, DB_PORT);
    
    if ($supabase_conn->connect_error) {
        die('Supabase Connection Failed: ' . $supabase_conn->connect_error);
    }
} catch (Exception $e) {
    die('Error connecting to Supabase: ' . $e->getMessage());
}

// Include legacy config if it exists
